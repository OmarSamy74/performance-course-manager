...
// Initialize database schema for Netlify functions
async function initDatabase() {
  try {
    ...
    if (checkResult.rows[0].exists) {
      console.log('✅ Database schema already exists (Netlify)');
      return;
    }
    
    console.log('📝 Creating database schema (Netlify)...');
    
    // Enable UUID extension
    await dbPool.query('CREATE EXTENSION IF NOT EXISTS "uuid-ossp"');
    
    // Create all tables (same as server/db/connection.ts)
    await dbPool.query(`
    ...
    // Create indexes
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_sessions_token ON sessions(token)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_sessions_user_id ON sessions(user_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_students_user_id ON students(user_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_installments_student_id ON installments(student_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_leads_status ON leads(status)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_submissions_student_id ON submissions(student_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_submissions_assignment_id ON submissions(assignment_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_quiz_attempts_student_id ON quiz_attempts(student_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_quiz_attempts_quiz_id ON quiz_attempts(quiz_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_progress_student_id ON progress(student_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_progress_lesson_id ON progress(lesson_id)');
    await dbPool.query('CREATE INDEX IF NOT EXISTS idx_grades_student_id ON grades(student_id)');
    
    console.log('✅ Database schema initialized (Netlify)');
  } catch (error: any) {
    console.error('❌ Error initializing database (Netlify):', error);
    // Don't throw - allow functions to continue
  }
}
...
export async function createById<T extends { id?: string }>(
  ...
  try {
    const dbPool = await getPool();
    if (!dbPool) throw new Error('Database pool not available');
